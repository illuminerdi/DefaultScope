...
<cffunction name="$IsValidFindAllArgument">
	...
	<cfscript>
		if (ListFindNoCase("where,order,select,distinct,include,maxRows,page,perPage,count,handle,cache,reload,parameterize,returnAs,returnIncluded",arguments.scopeName))
			return true;
		return false;
	</cfscript>
</cffunction>
